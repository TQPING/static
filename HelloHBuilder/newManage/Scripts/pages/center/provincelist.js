...
var ManageMent = {
    LoadData: function () {
        var Param = { ProvinceCode: $("#provinceIds").val(), CenterName: $("#province_search").val() };
        $("#infoPage").page({
            url: '/Center/ProvinceLoadList',
            pageSize: 10,
            searchparam: Param,
            viewCallback: ManageMent.outList
        });
    },
    ...
    infoDetail: function (centerID) {
        $.ajax({
            url: "/MainRoleChange/GetMainRoleChange",
    ...
    loading: function () {
        page = ManageMent.getQueryString("page");
        if (agentFlag == 0) {
            var area2 = new LArea();
            area2.init({
                'trigger': '#provinceVal', //触发选择控件的文本框，同时选择完毕后name属性输出到该位置
                'valueTo': '#provinceIds', //选择完毕后id属性输出到该位置
                'keys': {
                    id: 'id',
                    name: 'name'
            ...
            $("#provinceVal").cityPicker({
                toolbarTemplate: '<header class="bar bar-nav">\
            <button class="button button-link pull-right close-picker">确定</button>\
            <h4 class="title">选择所在地址</h4>\
            </header>',
                province: '#provinceIds',//省输入框,一般都是隐藏的，获取身份ID
                place: '#provinceIds2' //城市输入框，一般都是隐藏的，获取城市ID
            });
        }
    },
